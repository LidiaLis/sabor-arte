...
<title>Sabor &amp Arte — Recuperar senha</title>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .btn-login {
    ...
    animation: fadeUp 0.6s 0.42s both; position: relative; overflow: hidden;
  ...
  .divider { display: flex; align-items: center; gap: 14px; margin: 24px 0; animation: fadeUp 0.6s 0.48s both; }
  ...
  .btn-voltar {
    ...
    animation: fadeUp 0.6s 0.52s both; transition: background 0.2s, border-color 0.2s;
  }
  .btn-voltar:hover { background: rgba(74,94,58,0.06); border-color: var(--moss-light); }

  .version-tag { text-align: center; margin-top: auto; padding-top: 20px; animation: fadeUp 0.6s 0.58s both; }
...
<div class="login-card">
  ...
  <div class="card-header">
    ...
    <div class="card-title">Esqueceu sua <em>senha</em>?</div>
    <div class="card-subtitle">Sem problemas. Digite seu e-mail cadastrado e enviaremos um código para você criar uma nova senha.</div>
  </div>
  ...
  <div class="card-body">
    ...
    <form id="formEsqueci" action="#" method="post">

      <div class="form-group">
        <label class="form-label">E-mail cadastrado</label>
        <div class="input-wrapper">
          <span class="input-icon">✉️</span>
          <input type="email" class="form-input" id="emailInput" name="email" placeholder="seu@email.com" autocomplete="email" required autofocus>
        </div>
      </div>

      <button type="submit" class="btn-login" id="btnEnviar">
        Enviar código de recuperação <span class="btn-arrow">→</span>
      </button>
    ...
    <div class="divider">
      ...
      <span class="divider-text">Lembrou a senha?</span>
      <div class="divider-line"></div>
    </div>

    <a class="btn-voltar" href="${pageContext.request.contextPath}/LoginController">
      ← Voltar para o login
    </a>
...
<script>
  ...
  var form    = document.getElementById('formEsqueci');
  ...
  form.addEventListener('submit', function(e) {
    ...
    if (!emailRegex.test(valor)) {
      errMsg.textContent = '⚠️ Digite um e-mail válido.';
    ...
    fetch(contextPath + '/RecuperacaoSenhaController', {
      ...
      body: 'action=enviarCodigo&email=' + encodeURIComponent(valor)
    })
    .then(function(resp) {
    ...
    .then(function(res) {
      ...
      sucMsg.textContent = '✅ Enviamos um código de 6 dígitos para o e-mail ' + valor + '.';
      ...
      setTimeout(function() {
        window.location.href = contextPath + '/pages/verificar-codigo.jsp';
      }, 1600);
    })
    .catch(function() {
