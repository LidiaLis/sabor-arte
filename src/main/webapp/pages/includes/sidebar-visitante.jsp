...
<%
    ...
    String  _nome = (_u != null) ? _u.getNome_usuario() : "Visitante";
    String  _ini  = _nome.substring(0, 1).toUpperCase();
    String  _ctx  = request.getContextPath();

    /* badge de não lidas vindo da sessão ou atributo — ajuste conforme sua implementação */
    ...
    if (currentPage == null) {
        ...
        if      (sp.contains("receita"))     currentPage = "receitas";
        else if (sp.contains("autor"))       currentPage = "autores";
        else if (sp.contains("favorito"))    currentPage = "favoritos";
        else if (sp.contains("perfil"))      currentPage = "perfil";
        else if (sp.contains("notificacao")) currentPage = "notificacoes";
        else                                 currentPage = "receitas";
    }
%>
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-user" onclick="irParaPerfil()" style="cursor:pointer;" title="Meu perfil">
        <div class="user-avatar"><%= _ini %></div>
        <div class="user-info">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<%= _ctx %>/FavoritoController"
           class="nav-item <%= "favoritos".equals(currentPage) ? "active" : "" %>">
            <span class="nav-icon">❤️</span>
        ...
        <a href="<%= _ctx %>/NotificacaoController"
           class="nav-item <%= "notificacoes".equals(currentPage) ? "active" : "" %>">
            <span class="nav-icon">🔔</span>
            <span class="nav-label">Notificações</span>
            <% if (_notifNr > 0) { %>
                <span class="nav-badge"><%= _notifNr > 99 ? "99+" : _notifNr %></span>
            <% } %>
        </a>
...
<style>
    ...
    .sidebar-overlay {
        ...
        background: rgba(30,39,24,.45); z-index: 99;
    ...
    .user-avatar { width: 38px; height: 38px; background: linear-gradient(135deg, var(--gold), var(--gold-light)); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-family: 'Nunito', sans-serif; font-weight: 800; font-size: 14px; color: var(--moss-dark); flex-shrink: 0; }
